...
public static class KMeans {
    ...
    public static KMeansResults Cluster(Color[] items, int clusterCount, int maxIterations, int seed)
    {
        double[][] data = new double[items.Length][];
        for (int i = 0; i < items.Length; i++)
        {
            
            Vector3 v = new Vector3(items[i].r, items[i].g,items[i].b) ;
            data[i] = new double[] { v.x, v.y, v.z };
        }
        return Cluster(data, clusterCount, maxIterations, seed);
    }

    public static KMeansResults Cluster(GameObject[] items, int clusterCount, int maxIterations, int seed)
    {
        double[][] data = new double[items.Length][];
        for (int i = 0; i < items.Length; i++)
        {
