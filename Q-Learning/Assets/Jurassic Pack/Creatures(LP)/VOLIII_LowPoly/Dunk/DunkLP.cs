...
public class DunkLP : Creature
{
  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Slip,Bite,Swallow,Largesplash;
	...
	void PlaySound(string name, int time)
	{
		if(time==currframe && lastframe!=currframe)
		{
			...
			case "Bite": source[0].pitch=Random.Range(0.25f, 0.3f); source[0].PlayOneShot(Bite, 0.5f);
				lastframe=currframe; break;
    ...
    else
    {
      ...
      OnJump=true; body.drag=0.75f; body.angularDrag=0.75f; ApplyGravity();
		...
		{
			if(OnAnm.IsName("Dunk|SwimFastAtk"))  { PlaySound("Bite", 8); OnAttack=true; }
			else { PlaySound("Bite", 2);  PlaySound("Bite", 5);  OnAttack=true; }
			PlaySound("Swim",  (int) currframe);
		}
		...
		//Various
		else if(OnAnm.IsName("Dunk|Eat")) { OnReset=true; IsConstrained=true; PlaySound("Food", 2); PlaySound("Bite", 3); PlaySound("Bite", 6); }
    else if(OnAnm.IsName("Dunk|IdleC")) { PlaySound("Bite", 4); PlaySound("Bite", 6);PlaySound("Bite", 9); }
		else if(OnAnm.IsName("Dunk|Die-")) IsDead=false;
  ...
  //*************************************************************************************************************************************************
	...
	void LateUpdate()
	{
		...
		Spine0.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Spine1.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Spine2.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Spine3.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
    Spine4.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Spine5.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Spine6.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
    Spine7.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Head.transform.rotation*= Quaternion.Euler(spineY, 0, spineX);
		Tail0.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail1.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail2.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail3.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail4.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail5.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail6.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail7.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
		Tail8.transform.rotation*= Quaternion.Euler(-spineY, 0, -spineX);
