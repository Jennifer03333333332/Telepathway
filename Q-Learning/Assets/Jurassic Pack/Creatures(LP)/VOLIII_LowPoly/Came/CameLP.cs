...
{
	public Transform Root,Body,Tentacles,Right0,Right1,Right2,Right3,Right4,Right5,Right6,Right7,Right8,Left0,Left1,Left2,Left3,Left4,Left5,Left6,Left7,Left8;
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Bigstep,Largesplash,Ammo1,Ammo2,Ammo3;
	...
	void PlaySound(string name, int time)
	{
		if(time==currframe && lastframe!=currframe)
		{
			...
			case "Swim": source[1].pitch=Random.Range(0.5f, 0.75f); 
				if(IsOnWater && IsOnGround) source[1].PlayOneShot(Largesplash, 0.5f);
				else if(IsOnGround && !IsInWater) source[1].PlayOneShot(Bigstep, 0.5f);
				else if(IsOnWater) source[1].PlayOneShot(Waterflush, 0.1f);
				lastframe=currframe; break;
			case "Atk":int rnd = Random.Range(0, 2); source[0].pitch=Random.Range(0.6f, 0.75f);
				if(rnd==0) source[0].PlayOneShot(Ammo1, 0.1f);
				else source[0].PlayOneShot(Ammo2, 0.1f);
				lastframe=currframe; break;
			case "Die": source[0].pitch=Random.Range(0.6f, 0.75f); source[0].PlayOneShot(Ammo3, 0.1f);
    ...
    { 
      AICore(1, 2, 3, 0, 4, 0, 5); if(behavior.EndsWith("Hunt") | behavior.EndsWith("Food") | behavior.Equals("Battle"))
      OnInvert=true; else OnInvert=false;
    }
    ...
    //Set Y position
      ...
      if(Health!=0&&!OnAnm.IsName("Came|ToHide")&&!OnAnm.IsName("Came|ToHide-"))
			{
    ...
    //Various
		else if(OnAnm.IsName("Came|Eat")) { OnReset=true; IsConstrained=true; PlaySound("Atk", 1); }
		else if(OnAnm.IsName("Came|ToHide") | OnAnm.IsName("Came|ToHide-") ) OnReset=true;
		else if(OnAnm.IsName("Came|Die-")) { OnInvert=false; PlaySound("Atk", 1); IsDead=false; }
