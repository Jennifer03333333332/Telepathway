  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Slip,Bite,Swallow,Smallsplash;
    ...
    else
    {
      ...
      OnJump=true; body.drag=0.5f; body.angularDrag=0.5f; ApplyGravity();
		...
		{
			if(OnAnm.IsName("Xena|SwimFastAtk"))  { PlaySound("Bite", 8); OnAttack=true; }
      ...
      Move(transform.forward, 15);
		...
		//Various
		else if(OnAnm.IsName("Xena|Eat"))  { IsConstrained=true; PlaySound("Food", 2); }
		else if(OnAnm.IsName("Xena|Die-")) IsDead=false;
