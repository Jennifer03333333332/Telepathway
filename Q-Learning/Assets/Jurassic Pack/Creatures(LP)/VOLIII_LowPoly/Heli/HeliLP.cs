...
public class HeliLP : Creature
{
  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Slip,Bite,Swallow,Smallsplash;
    ...
    else
    {
      ...
      OnJump=true; body.drag=0.75f; body.angularDrag=0.75f; ApplyGravity();
		...
		{
			if(OnAnm.IsName("Heli|SwimFastAtk"))  { PlaySound("Bite", 8); OnAttack=true; }
		...
		//Various
		else if(OnAnm.IsName("Heli|Eat")) { OnReset=true; IsConstrained=true; PlaySound("Food", 2); }
		else if(OnAnm.IsName("Heli|Die-")) IsDead=false;
